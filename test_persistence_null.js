...
async function testPersistence() {
  console.log('--- TESTE DE PERSISTÊNCIA (NULL TENANT) ---');
  ...
  const resInsert = await fetch(`${url}/rest/v1/employees`, {
    ...
    body: JSON.stringify({ name: 'TESTE PERSISTENCIA NULL', cpf: cpf, status: 'ACTIVE', tenant_id: null })
  });
  ...
  // 3. Verificar
  console.log('Verificando (esperando deleção persistir)...');
